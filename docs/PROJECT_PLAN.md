...
M3 completes the playable wave-composer and scripted-match loop. `WaveDraft` owns application-local entries, stable draft IDs, catalog-derived costs, routes, 5/15/30-tick spacing, and undo/redo snapshots; no draft gesture mutates `MatchState`. Commit normalizes and validates a frozen schedule before the coordinator begins fixed-tick resolution. Pause and 1x/2x/4x affect only presentation tick delivery. Analysis is derived from combat events, and the flow supports next-round progression, player/defender terminal outcomes, and restart from the same root seed. Automated and live-runtime evidence is recorded in `docs/M3_VERIFICATION.md`.
...
M0.1 ratifies these decisions as one combined v0 scope with the non-goals in this plan and the capability trace in the MVP development plan. M1.0 ratified the simulation architecture baseline, fixed-tick direction, content policy, initial testing direction, scene boundary, and replay scope. M1 completed the focused ADRs and executable foundation evidence on 2026-07-24. M2.1 implements the ratified movement/leak rules, M2.2 implements inclusive targeting, all four comparator families, staged attacks, cadence, armor/penetration, splash, Control Slow, overkill/death order, and retargeting, M2.3 implements proximity Rally plus real-content scenarios for every initial unit/tower strength and weakness, M2.4 implements semantic combat telemetry plus checked seeded scenario artifacts, and M2.5 verifies the measured 300-unit/100-tower simulation and current presentation-proxy envelope. M3 is complete locally. M4 is complete locally: copied fairness-filtered history, bounded legal command planning, dedicated controlled variation, decision diagnostics, between-round player-facing adaptation, restart clearing, and red-team contracts are recorded in [M4 Adaptive Defender Verification](M4_VERIFICATION.md). Theme/tone and detailed presentation direction remain open only where they do not change these product or architecture contracts.
