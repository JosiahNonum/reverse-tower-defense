...
func test_map_selection_updates_tower_details_and_priority_reads() -> void:
	...
	var towers: Array[DefenseTowerInspection] = main.get_defense_inspection_model().get_towers()
	var selected: DefenseTowerInspection = towers[0]
	assert_equal(panel.get_selected_tower_id(), selected.get_tower_id())
	assert_true(map_view.select_tower(selected.get_slot_id()))
	assert_equal(map_view.get_selected_slot_id(), selected.get_slot_id())
	assert_equal(panel.get_selected_tower_id(), selected.get_tower_id())
	assert_false(panel.get_policy_text().is_empty())
	assert_false(panel.get_routes_text().is_empty())
	assert_false(panel.get_threat_summary_text().is_empty())
	map_view.size = Vector2(760, 500)
	var click := InputEventMouseButton.new()
	click.button_index = MOUSE_BUTTON_LEFT
	click.pressed = true
	click.position = map_view.get_tower_screen_position(selected.get_slot_id())
	map_view._gui_input(click)
	assert_equal(panel.get_selected_tower_id(), selected.get_tower_id())
	map_view.select_relative_tower(1)
	assert_true(panel.get_selected_tower_id() != &"")
