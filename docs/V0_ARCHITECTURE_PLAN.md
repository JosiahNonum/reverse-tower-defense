...
The current `Main` composition supplies the scripted-defense match reveal and routes player waves through the coordinator's validated schedule seam. Responsive containers retain the 1280x720 base and are visually checked at 1440x900 and 1024x768; inspection details scroll rather than changing rule state or hiding failures.

M3's application-owned `WaveDraft` seam stores editable `WaveDraftEntry` values with temporary stable IDs, allowed unit/route/spacing data, and reversible snapshots. It derives cost and round budget from `ContentCatalog` plus `MatchRulesDefinition`, reports a typed `WaveDraftValidation`, and has no reference to `MatchState`, presentation, or UI. The `WaveComposerPanel` consumes this draft contract. Atomic commit turns copied entries into a schedule validated by the coordinator before a fixed-defense simulation is created; the editor never spends budget or mutates the match state directly.
